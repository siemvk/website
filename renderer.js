...
// Configure marked so internal links become <weblink> elements
...
mdRenderer.link = function (token) {
    ...
    try {
        ...
        if (linkUrl.host === window.location.host) {
            return `<weblink href="${href}">${text}</weblink>`;
        } else {
            const titleAttr = title ? ` title="${title}"` : "";
            return `<a href="${href}" target="_blank" rel="noopener noreferrer"${titleAttr}>${text}</a>`;
        }
    } catch (e) {
        // If URL parsing fails, fall back to weblink for relative/unknown links
        return `<weblink href="${href}">${text}</weblink>`;
    }
};
...
async function run() {
    ...
    const initialPage = url.searchParams.get("page") || config.homePage;
    await goTo(initialPage, { pushHistory: false });
    window.addEventListener("popstate", () => {
        const currentUrl = new URL(window.location.href);
        const page = currentUrl.searchParams.get("page") || config.homePage;
        goTo(page, { pushHistory: false });
    });
}
